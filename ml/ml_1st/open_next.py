...
# 오늘의 데이터를 이용해 다음날의 시가를 예측해야하므로 하루씩 뒤로 밀기
...
plt.plot(result_df.index, result_df['Actual'], label='Actual Next Open', color='blue', linewidth=2)
plt.plot(result_df.index, result_df['Predicted'], label='Predicted Next Open', color='red', linestyle='--', linewidth=2)

plt.title('Test Set: Actual vs. Predicted Next Open Price (Regression)')
plt.xlabel('Date')
