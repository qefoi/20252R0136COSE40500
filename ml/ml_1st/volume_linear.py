...
# 오늘의 데이터를 이용해 다음날의 거래량을 예측해야하므로 하루씩 뒤로 밀기
...
plt.plot(result_df.index, result_df['Actual'], label='Actual Next Volume', color='blue', linewidth=2)
plt.plot(result_df.index, result_df['Predicted'], label='Predicted Next Volume', color='red', linestyle='--', linewidth=2)

plt.title('Test Set: Actual vs. Predicted Next Volume (Regression)')
plt.xlabel('Date')
