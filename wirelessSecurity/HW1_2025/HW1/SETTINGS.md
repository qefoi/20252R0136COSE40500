# HW1 Development Environment Setup Guide

This document provides instructions for setting up the development environment for the HW1 assignment. You can choose between Docker environment or local environment setup.
...
# Access container
docker-compose exec hw1-environment bash
...
docker run -d --name hw1-container \
  -e MY_UID=$(id -u) \
  -e MY_GID=$(id -g) \
  -v $(pwd):/homework \
  hw1-env tail -f /dev/null

# Access container
docker exec -it hw1-container bash

# Cleanup after work completion
docker stop hw1-container && docker rm hw1-container
```
...
docker run -it --user root -v $(pwd):/homework hw1-env
```
...
docker run -it -e MY_UID={YOUR_UID} -e MY_GID={YOUR_GID} -v $(pwd):/homework hw1-env
```
...
docker build --no-cache -t hw1-env .
```
...
export MY_UID=$(id -u) MY_GID=$(id -g)
docker build -t hw1-env .
docker run --rm -e MY_UID=$MY_UID -e MY_GID=$MY_GID -v $(pwd):/homework hw1-env bash -c "cd src && make all && ./tests"
```
...
docker run -d --name hw1-dev -e MY_UID=$(id -u) -e MY_GID=$(id -g) -v $(pwd):/homework hw1-env tail -f /dev/null

# Development work
docker exec -it hw1-dev bash
