...
START_TEST(grayscale_examples) {
  ...
  ck_assert_uint_eq(img->size_x,img_gray->size_x);
  ck_assert_uint_eq(img->size_y,img_gray->size_y);

  for (long j = 0;j < img->size_x*img->size_y;j++){
    ck_assert_uint_eq(img->px[j].red,img_gray->px[j].red);
    ck_assert_uint_eq(img->px[j].green,img_gray->px[j].green);
    ck_assert_uint_eq(img->px[j].blue,img_gray->px[j].blue);
  }
}
END_TEST

START_TEST(grayscale_null_image) {
  double weights[] = {0.2125, 0.7154, 0.0721};
...
START_TEST(negative_functionality) {
  ...
  uint16_t a = img.size_x;
  uint16_t b = img.size_y;
  filter_negative(&img,NULL);
  ck_assert_uint_eq(a,img.size_x);
  ck_assert_uint_eq(b,img.size_y);
...
START_TEST(blur_functionality) {
  ...
  int radius=0;
  filter_blur(&dup_img,&radius);
  for (int i=0;i<3;i++){
    for (int j=0;j<3;j++){
      int idx=i*3+j;
      ck_assert_uint_eq(px[i][j].red,dup_img.px[idx].red);
      ck_assert_uint_eq(px[i][j].green,dup_img.px[idx].green);
      ck_assert_uint_eq(px[i][j].blue,dup_img.px[idx].blue);
    }
  }
  free(dup_img.px);
  ...
  for (int i=0;i<3;i++){
    for (int j=0;j<3;j++){
      int idx=i*3+j;
      ck_assert_uint_eq(result1[i][j].red,dup_img.px[idx].red);
      ck_assert_uint_eq(result1[i][j].green,dup_img.px[idx].green);
      ck_assert_uint_eq(result1[i][j].blue,dup_img.px[idx].blue);
    }
  }
  ...
  filter_blur(&dup_img,&radius);
    ...
    for (int j=0;j<3;j++){
      int idx=i*3+j;
      ck_assert_uint_eq(result2[i][j].red,dup_img.px[idx].red);
      ck_assert_uint_eq(result2[i][j].green,dup_img.px[idx].green);
      ck_assert_uint_eq(result2[i][j].blue,dup_img.px[idx].blue);
    }
  }
...
START_TEST(blur_radius_edge_cases) {
  struct image img_test = duplicate_img(blur_radius_img);
  int radius = blur_radii[_i];
  
  filter_blur(&img_test, &radius);
  
  ck_assert_uint_eq(img_test.size_x, blur_radius_img.size_x);
  ck_assert_uint_eq(img_test.size_y, blur_radius_img.size_y);
  ck_assert_ptr_ne(img_test.px, NULL);
...
START_TEST(transparency_functionality) {
  ...
  struct pixel *store = malloc(a*b*sizeof(struct pixel));
  memcpy(store,img.px,a*b*sizeof(struct pixel));

  uint8_t alpha = rand()%256;
  filter_transparency(&img, &alpha);

  ck_assert_uint_eq(img.size_x,a);
  ck_assert_uint_eq(img.size_y,b);

  for (long i=0;i<a*b;i++){
    ck_assert_uint_eq(img.px[i].red,store[i].red);
    ck_assert_uint_eq(img.px[i].green,store[i].green);
    ck_assert_uint_eq(img.px[i].blue,store[i].blue);
    ck_assert_uint_eq(img.px[i].alpha,alpha);
  }
...
START_TEST(transparency_edge_case) {
  ...
  ck_assert(0 && "Should have segfaulted");
}
END_TEST
...
int main() {
  ...
  tcase_add_test(tc1, grayscale_null_image);
  /* Tests for limits*/
