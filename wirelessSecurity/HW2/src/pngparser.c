...
struct image *convert_color_palette_to_image(png_chunk_ihdr *ihdr_chunk,
  ...
  if (!plte_chunk || !plte_chunk->chunk_data || plte_chunk->length == 0) return NULL;
  ...
  if (width != 0 && height > UINT32_MAX / width) goto error;

  img->px = malloc(sizeof(struct pixel) * img->size_x * img->size_y);

  if (!img->px) goto error;

  uint32_t num_plte = plte_chunk->length/3;

  if ((1 + width)*height > inflated_size) goto error;
  
  for (uint32_t idy = 0; idy < height; idy++) {
    // Filter byte at the start of every scanline needs to be 0
    if (idy *(1+width) >= inflated_size) goto error;
    if (inflated_buf[idy * (1 + width)]) goto error;

    for (uint32_t idx = 0; idx < width; idx++) {

      if (idy *(1+width) + idx + 1 >= inflated_size) goto error;

      palette_idx = inflated_buf[idy * (1 + width) + idx + 1];

      if (palette_idx >= num_plte) goto error;
...
struct image *convert_rgb_alpha_to_image(png_chunk_ihdr *ihdr_chunk,
  ...
  if (!img->px) goto error;
  if ((1 + 4 * (uint64_t)width)*height > inflated_size) goto error;
                                  
  for (uint32_t idy = 0; idy < height; idy++) {
    // The filter byte at the start of every scanline needs to be 0
    if (idy * (1 + 4 * width) >= inflated_size) goto error;
    if (inflated_buf[idy * (1 + 4 * width)]) goto error;
    ...
    for (uint32_t idx = 0; idx < width; idx++) {
      ...
      if (a_idx >= inflated_size) goto error;
